"""
Client for MUD (multi user dangeon).
...
def movemonsters(arg, socket):
    """Correctness checker for 'movemonsters' command."""
    if not arg:
        print("Error! Empty arguments")
        return
    arg = arg.strip()
    match arg:
        case 'on' | 'off':
            socket.sendall(f"movemonsters {arg}\n".encode())
        case _:
            print('Invalid argument')


def locale(arg, socket):
    """Correctness checker for 'locale' command."""
    if not arg:
        print("Error! Empty arguments")
        return
    arg = arg.strip()
    socket.sendall(f"locale {arg}\n".encode())
...
class MUDcmd(cmd.Cmd):
    ...
    def do_movemonsters(self, arg):
        """Turn on/off monsters moving.

        movemonsters on|off
        """
        movemonsters(arg, self.socket)

    def complete_movemonsters(self, text, line, begidx, endidx):
        """Complete command movemonsters."""
        cmd_len = len(shlex.split(line[:endidx] + '.'))
        if cmd_len == 2:
            return [c for c in ['on', 'off'] if c.startswith(text)]

    def do_locale(self, arg):
        """Set server message locale."""
        locale(arg, self.socket)
    
    def complete_locale(self, text, line, begidx, endidx):
        """Complete command locale."""
        cmd_len = len(shlex.split(line[:endidx] + '.'))
        if cmd_len == 2:
            return [c for c in ['en_US.UTF-8', 'ru_RU.UTF-8'] if c.startswith(text)]
